...
class VGG():
    ...
    def argmin_2d(self, matrix):
        n_filt = matrix.get_shape().as_list()[-1]
        x = tf.reshape(matrix, [n_filt*n_filt,])
        x_nozeroidx = tf.where(tf.not_equal(x, 0))
        x_suppressed = tf.gather_nd(x,x_nozeroidx)
        indices = x_nozeroidx[tf.argmin(x_suppressed)]    
        col_indices = indices // n_filt
        row_indices = indices % n_filt
        final_indices = tf.stack([col_indices[0], row_indices[0]])
    ...
    def _add_thomson_first(self, filt, n_filt):
        ...
        idx = self.argmin_2d(final)
        target = tf.gather_nd(final,idx)
        mask = tf.equal(final,target)
        loss = -tf.boolean_mask(final, mask)
        loss = 0.1 * tf.reduce_sum(loss)        
