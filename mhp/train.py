...
def train(base_lr, batch_sz, gpu_no, iters):
    ...
    vgg = VGG()
    vgg.build(images, n_class, is_training, lr_)
    ...
    reg_loss_list = [var for var in allreg if 'poles' not in var.name]
    vreg_loss_list = [var for var in allreg if 'poles' in var.name]
    ...
    vloss_op = thom_loss+thom_final
    vreg_loss = tf.add_n(vreg_loss_list)
    vloss_op += vreg_loss

    allvars = tf.trainable_variables()
    normal_vars = [var for var in allvars if 'poles' not in var.name]
    v_vars = [var for var in allvars if 'poles' in var.name]

    updatev_op = tf.train.MomentumOptimizer(lr_, 0.9).minimize(vloss_op, var_list=v_vars)  
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_epoch):
            ...
            for t in range(iters):
                sess.run(updatev_op,{lr_: lr, is_training: True, images: tr_images, labels: tr_labels})
...
if __name__ == "__main__":
    ...
    train(args.base_lr, args.batch_size, args.gpu_no, args.iters)
